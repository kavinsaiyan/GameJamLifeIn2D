    ...
    public class LevelSelectScreen
    {
        ...
        public LevelSelectScreen(LevelInfo levelInfo, int screenWidth, int screenHeight, ContentManager contentManager)
        {
            ...
            for (int i = 0; i < _levelsCount; i++)
            {
                ...
                {
                    // Logger.Instance.Log($"Save data for level {i+1} is {levelSaveItem.levelState}");
                    levelCard.LevelState = levelSaveItem.levelState;
                }
                else
                    Logger.Instance.LogError("[LevelSelectScree.cs/LevelSelectScree]: Save data not found for " + (i + 1));
